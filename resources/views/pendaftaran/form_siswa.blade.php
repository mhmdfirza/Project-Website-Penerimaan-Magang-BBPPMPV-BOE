...
<main class="flex-1 px-6 py-8">
    ...
    <form action="{{ route('siswa.store') }}" method="POST" enctype="multipart/form-data" class="max-w-3xl mx-auto">
        ...
        @for ($i = 0; $i < $jumlah_siswa; $i++) <div
            class="siswa-form hidden bg-white shadow-md rounded-lg p-6 space-y-6 border border-gray-200"
            data-step="{{ $i + 1 }}">
            ...
            <div>
                ...
                <input type="text" name="siswa[{{ $i }}][nama_siswa]" value="{{ old('siswa.' . $i . '.nama_siswa') }}"
                    required
            ...
            <div>
                ...
                <input type="text" name="siswa[{{ $i }}][kelas]" value="{{ old('siswa.' . $i . '.kelas') }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition"
            ...
            <div>
                ...
                <input type="tel" name="siswa[{{ $i }}][no_hp]" value="{{ old('siswa.' . $i . '.no_hp') }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition"
            ...
            <div class="md:col-span-2">
                ...
                <input type="file" id="foto_siswa_{{ $i }}" name="siswa[{{ $i }}][foto]" data-index="{{ $i }}"
                    accept="image/*" {{ empty($pendaftaran['siswa'][$i]['foto_temp'] ?? '') ? 'required' : '' }}
                ...
                <div id="preview_foto_siswa_{{ $i }}" class="mt-3">
                    ...
                    <div class="flex items-center space-x-4">
                        <img src="{{ Storage::url($pendaftaran['siswa'][$i]['foto_temp']) }}" alt="Preview Foto"
            ...
            <div>
                ...
                <textarea name="siswa[{{ $i }}][alamat_rumah]" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition"
                    required placeholder="Alamat lengkap rumah">{{ old('siswa.' . $i . '.alamat_rumah') }}</textarea>
                <p class="text-red-500 text-sm mt-1 hidden">Wajib diisi</p>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="siswa[{{ $i }}][tgl_mulai]" id="tgl_mulai_{{ $i }}" data-index="{{ $i }}"
                        value="{{ $pendaftaran['siswa'][$i]['tgl_mulai'] ?? session('pendaftaran.tgl_mulai') }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">TANGGAL SELESAI</label>

                    <input type="date" name="siswa[{{ $i }}][tgl_selesai]" id="tgl_selesai_{{ $i }}"
                        data-index="{{ $i }}"
                        value="{{ $pendaftaran['siswa'][$i]['tgl_selesai'] ?? session('pendaftaran.tgl_selesai') }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                </div>
            </div>
            </div>
            ...
            <div class="flex justify-between items-center mt-6">
                <div class="flex space-x-2">
                    <a href="{{ route('pembimbing.form') }}"
                        ...
                        < Form Pembimbing </a> <button type="button" id="prevBtn"
            ...
            <div class="flex justify-center mt-6 space-x-4">
                @for ($i = 1; $i <= $jumlah_siswa; $i++) <button type="button"
                ...
                <div class="mt-2 px-7 py-3">
                    ...
                    <div class="mt-4 text-left bg-gray-50 p-3 rounded-lg">
                        ...
                        <div id="confirmationDetails" class="text-xs text-gray-600 space-y-1">
                            ...
                            <p><strong>Status Validasi:</strong> <span id="validationStatus"
                                    class="text-green-600">Semua data valid</span></p>
                        </div>
                    </div>
                </div>
